...
func (s *Service) SetRulesWithDelivery(ctx context.Context, ownerUserID int64, key domain.PrivacyKey, rules []domain.PrivacyRule, build store.PrivacyDeliveryPayloadBuilder, excludeAuthKeyID [8]byte, excludeSessionID int64) (domain.PrivacyRules, error) {
	out, err := normalizedRules(ownerUserID, key, rules)
	if err != nil {
		return domain.PrivacyRules{}, err
	}
	if s == nil || s.rules == nil {
		return domain.PrivacyRules{}, store.ErrPrivacyDeliveryStoreMissing
	}
	delivery, ok := s.rules.(store.PrivacyDeliveryStore)
	if !ok {
		return domain.PrivacyRules{}, store.ErrPrivacyDeliveryStoreMissing
	}
	if err := delivery.SetPrivacyRulesWithDelivery(ctx, out, build, excludeAuthKeyID, excludeSessionID); err != nil {
		return domain.PrivacyRules{}, err
	}
	return out, nil
}
