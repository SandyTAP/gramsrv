...
func TestPaidReactionAtomicLedgerMigrationCreditsLegacyAggregatesPostgres(t *testing.T) {
	...
	downSQL, err := deploy.Migrations.ReadFile("migrations/0183_paid_reaction_atomic_ledger.down.sql")
	if err != nil {
		t.Fatalf("read 0183 down: %v", err)
	}
	upSQL, err := deploy.Migrations.ReadFile("migrations/0183_paid_reaction_atomic_ledger.up.sql")
	if err != nil {
		t.Fatalf("read 0183 up: %v", err)
	}
	...
	if _, err := tx.Exec(ctx, string(downSQL)); err != nil {
		t.Fatalf("apply 0183 down fixture: %v", err)
	}
	const (
...
RETURNING id`).Scan(&transactionID); err != nil {
	...
	if downErr == nil || !strings.Contains(downErr.Error(), "cannot downgrade paid reaction atomic ledger after post-upgrade activity") {
		t.Fatalf("0183 down error=%v, want post-upgrade activity guard", downErr)
	}
	...
	if commandsTable == "" {
		t.Fatal("rejected 0183 down discarded the paid reaction command table")
	}
}
