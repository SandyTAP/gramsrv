...
VALUES($1,9184001,9184002,'stars',1,'completed',1700000900,1700000000,
	...
	downSQL, err := deploy.Migrations.ReadFile("migrations/0184_channel_revenue_withdrawals.down.sql")
	if err != nil {
		t.Fatalf("read 0184 down: %v", err)
	}
	tx, err := pool.Begin(ctx)
	if err != nil {
		t.Fatal(err)
	...
	if downErr == nil || !strings.Contains(downErr.Error(), "cannot downgrade channel revenue withdrawals after a completed claim") {
		t.Fatalf("0184 down error=%v, want completed-claim guard", downErr)
	}
	var receipts int
