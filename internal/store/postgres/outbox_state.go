...
var (
	dispatchOutboxStateConfig = outboxStateConfig{
...
func (s *durableOutboxState) ClaimWindows(ctx context.Context, req store.OutboxClaimRequest) ([]store.OutboxClaimWindow, error) {
	...
	if scoped {
		shardPredicate = "l.logical_shard = ANY($8::smallint[])"
...
LIMIT LEAST($4, GREATEST(1, ($5::bigint / 65536)::integer))`, s.cfg.sequenceSQL, s.cfg.payloadSQL, s.cfg.recoverySQL, s.cfg.orderSQL, s.cfg.orderSQL, s.cfg.orderSQL, s.cfg.itemsTable)
...
ORDER BY e.ordinal`, s.cfg.attemptsTable, s.cfg.lanesTable,
		s.cfg.targetsTable, s.cfg.attemptsTable, s.cfg.targetsTable), payload)
	if err != nil {
