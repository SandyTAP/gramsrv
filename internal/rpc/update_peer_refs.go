...
func (r *Router) enrichPreparedUpdateEvents(ctx context.Context, viewerUserID int64, events []domain.UpdateEvent, cache *viewerPeerCache, strictUsers bool) ([]domain.UpdateEvent, error) {
	...
	for i := range out {
		...
		if strictUsers {
			// Durable store envelopes may contain unprojected base users that are
			// not discoverable from an older event constructor. Treat their IDs as
			// expected, but never retain their fields in the response.
	...
	for i := range out {
		if strictUsers {
			...
			// Replace the raw durable envelope instead of merging it. A merge would
			// preserve an account phone/access hash whenever projection omitted an
			// expected user, exactly when the response must fail closed.
			out[i].Users = users
		} else {
...
func (r *Router) enrichMessageList(ctx context.Context, viewerUserID int64, list domain.MessageList) domain.MessageList {
	...
	if r.messageUsersAreViewerProjected() {
		// The application service already applied contact/photo/privacy projection
		// for these users. Prime the batch cache so only nested refs absent from the
		// store envelope require another projection query.
