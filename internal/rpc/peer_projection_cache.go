...
// usersForIDsStrict resolves a durable response envelope. Unlike the ordinary
// logging wrapper above, it rejects both resolver errors and successful but
// incomplete results so callers cannot fall back to raw store users while
// advancing a difference cursor.
...
// primeUsers preheats known projected users when the caller does not own a
...
// primeExpectedUsers preheats a complete ByIDsForViewers result. Callers first
// validate that every non-system expected ID is present; the negative entries
// keep a later builder from silently issuing scalar lookups for protocol-local
// system IDs or a future explicitly optional reference.
...
// missingProjectedUserID verifies the complete batch-projection contract. A
// durable message reference may point at a deleted tombstone, but it must not
// silently lose its user envelope. System users are synthesized locally and
// therefore need not be present in the resolver result.
