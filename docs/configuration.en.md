...
| `TELESRV_EGRESS_DELIVERY_GRPC_ADDR` | address / empty | Internal listen address for the late client-ACK writeback API on `cmd/telesrv-egress`. The Egress entrypoint requires this setting. Service-level Edge fabric confirmation is enough to advance online outbox rows; this API is only a late signal for indeterminate writes and supports both PTS `dispatch_outbox` and non-PTS `edge_delivery_outbox` (the non-PTS lane uses `pts=0` in the fence). |
| `TELESRV_EGRESS_DELIVERY_GRPC_RESOLVER` | `static`/`dns` / `static` | Edge -> Egress Delivery gRPC service-discovery provider. It follows the CoreExec resolver rules: `static` consumes comma-separated `host:port` endpoints, while `dns` consumes exactly one bare `host:port` or explicit `dns:///name:port` / `dns://authority:port/name:port` target. Unsupported values fail startup. |
| `TELESRV_EGRESS_DELIVERY_GRPC_TARGETS` | resolver target(s) / empty | Required Egress Delivery gRPC endpoint input for `cmd/telesrv-edge`. Edge performs health + `GetInfo` before publishing the MTProto listener; unreachable Egress Delivery, token errors, or protocol-version mismatch fail fast so clients cannot connect to an Edge that cannot advance PTS outbox or non-PTS delivery queue ACK state. |
| `TELESRV_EGRESS_DELIVERY_GRPC_REQUEST_TIMEOUT` | duration / `5s` | Default deadline for Edge -> Egress Delivery gRPC batch calls. It is added only when the upstream context has no deadline. Client ACKs use one bounded micro-batch path with no legacy unary fallback. Must be greater than `0` and at most `1m`. |
| `TELESRV_EGRESS_DELIVERY_GRPC_TLS_CERT_FILE` / `TELESRV_EGRESS_DELIVERY_GRPC_TLS_KEY_FILE` | path / empty | Egress Delivery gRPC server TLS certificate and private key. They must be configured together. Empty keeps the local/development default plaintext gRPC channel. |
| `TELESRV_EGRESS_DELIVERY_GRPC_TLS_CLIENT_CA_FILE` | path / empty | CA bundle used by the Egress Delivery gRPC server to verify Edge client certificates. Setting it enables mTLS and requires the server TLS cert/key settings. |
| `TELESRV_EGRESS_DELIVERY_GRPC_TLS_CA_FILE` | path / empty | Root CA bundle trusted by the Edge gRPC client when verifying Egress Delivery server certificates. Setting it enables TLS on the client. |
| `TELESRV_EGRESS_DELIVERY_GRPC_TLS_SERVER_NAME` | string / empty | Server name used by the Edge gRPC client when verifying the Egress Delivery server certificate. A non-empty value also enables TLS. |
| `TELESRV_EGRESS_DELIVERY_GRPC_TLS_CLIENT_CERT_FILE` / `TELESRV_EGRESS_DELIVERY_GRPC_TLS_CLIENT_KEY_FILE` | path / empty | Edge gRPC client certificate and private key for Egress Delivery calls. They must be configured together. |
| `TELESRV_EGRESS_DELIVERY_TOKEN` | secret string / empty | Bearer token for Egress Delivery gRPC APIs. `cmd/telesrv-edge` and `cmd/telesrv-egress` both require an explicit non-empty value, and it must not contain newlines. |
...
| `TELESRV_POSTGRES_COUNTER_RECOVERY_MAX_CONNS` | positive int / `4` | Maximum size of the PostgreSQL pool dedicated to Redis box/channel counter cold recovery. It must be independent from the business transaction pool; Core startup fails for non-positive values. YAML: `postgres.counter_recovery_max_conns`. |
...
| `TELESRV_DELIVERY_ATTEMPT_TIMEOUT` | duration / `2s` | Absolute Edge command lifetime frozen by the database at claim time. Projection, Redis queuing, Edge admission, and the final socket write share the same non-renewable `CommandNotAfter`. |
| `TELESRV_DELIVERY_CLOCK_SKEW_ALLOWANCE` | duration / `1s` | Cross-host clock-skew budget after the wire fence. A fresh fence is not eligible until the later database `EvidenceDeadline`; must be positive and at most `10s`. |
| `TELESRV_OUTBOUND_PUSH_TIMEOUT` | duration / `200ms` | Per-call Redis/remote admission I/O bound. Durable correctness uses the separately frozen delivery-attempt deadline. |
| `TELESRV_SEND_RATE_LIMIT` | int / `0` | Optional per-account send policy, disabled by default. A positive value enables window counting and standard `FLOOD_WAIT_n` rejection; durable admission/backpressure independently protects system capacity. |
